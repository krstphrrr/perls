...
if($search_type == 'Sites')
{
	...
	$pagelist = $p->pageList($_GET['page'], $pages,$search_type,$select_multiple_sites,$site_elevlow_min,$site_elevlow_max,$site_elevhigh_min,$site_elevhigh_max,$site_temp_min,$site_temp_max,$site_precip_min,$site_precip_max,$sort_type); 
}
...
else if($search_type == 'Search')
{
	...
	$pagelist = $p->pageList($_GET['page'], $pages, $search_type, $search_results); 
}
...
else
{
	...
	$pagelist = $p->pageList($_GET['page'], $pages,$search_type,$select_multiple_sites,$site_elev_min,$site_elev_max,$site_temp_min,$site_temp_max,$site_precip_min,$site_precip_max,sort_type); 
...
<table width="850" border="0" cellspacing="0" cellpadding="5">
  ...
  <? if($num_results!=0){ ?>
  <tr>
    <td width="609">There are <? echo "$count"; ?> sites that meet your criteria. Click on Details to see more detailed information about a site. Click on Map to show the site on the map above. Click on the site name to go to the site's homepage. You may also click on any of the site markers in the map above.</td>
    <td width="85"><form name="csv" action="export/csv.php" enctype="multipart/form-data" method="post">
  <input type="hidden" name="big_var" value="<? echo "$big_var"; ?>">
    CSV 
    <div align="right">
...
EXCEL
 <div align="right">
  ...
  <tr>
    <td colspan="2"><? if($count>=$limit){ echo "$pagelist"; } ?></td>
  </tr>
  <tr>
    <td colspan="2"><table width="100%" cellpadding="3" cellspacing="0">
        <tr>
          <td width="11%">&nbsp;</td>
          <td width="10%"><? echo"<a href=\"javascript:searchSiteCriteriaTable2('?page=".$change_sort_page."$i&search_type=$search_type&search_multiple_sites=$search_multiple_sites&site_elev_min=$site_elev_min&site_elev_max=$site_elev_max&site_temp_min=$site_temp_min&site_temp_max=$site_temp_max&site_precip_min=$site_precip_min&site_precip_max=$site_precip_max&sort_type=site_name')\" ><b>Site Name</b></a>";?></td>
          <td width="10%"><? echo"<a href=\"javascript:searchSiteCriteriaTable2('?page=".$change_sort_page."$i&search_type=$search_type&search_multiple_sites=$search_multiple_sites&site_elev_min=$site_elev_min&site_elev_max=$site_elev_max&site_temp_min=$site_temp_min&site_temp_max=$site_temp_max&site_precip_min=$site_precip_min&site_precip_max=$site_precip_max&sort_type=site_lat')\" ><b>Latitude</b></a>";?></td>
          <td width="13%"><? echo"<a href=\"javascript:searchSiteCriteriaTable2('?page=".$change_sort_page."$i&search_type=$search_type&search_multiple_sites=$search_multiple_sites&site_elev_min=$site_elev_min&site_elev_max=$site_elev_max&site_temp_min=$site_temp_min&site_temp_max=$site_temp_max&site_precip_min=$site_precip_min&site_precip_max=$site_precip_max&sort_type=site_long')\" ><b>Longitude</b></a>";?></td>
          <td width="11%"><? echo"<a href=\"javascript:searchSiteCriteriaTable2('?page=".$change_sort_page."$i&search_type=$search_type&search_multiple_sites=$search_multiple_sites&site_elev_min=$site_elev_min&site_elev_max=$site_elev_max&site_temp_min=$site_temp_min&site_temp_max=$site_temp_max&site_precip_min=$site_precip_min&site_precip_max=$site_precip_max&sort_type=site_elevlow')\" ><b>Elevation</b></a>";?></td>
          <td width="8%"><? echo"<a href=\"javascript:searchSiteCriteriaTable2('?page=".$change_sort_page."$i&search_type=$search_type&search_multiple_sites=$search_multiple_sites&site_elev_min=$site_elev_min&site_elev_max=$site_elev_max&site_temp_min=$site_temp_min&site_temp_max=$site_temp_max&site_precip_min=$site_precip_min&site_precip_max=$site_precip_max&sort_type=site_temp')\" ><b>Temp</b></a>";?></td>
          <td width="9%"><? echo"<a href=\"javascript:searchSiteCriteriaTable2('?page=".$change_sort_page."$i&search_type=$search_type&search_multiple_sites=$search_multiple_sites&site_elev_min=$site_elev_min&site_elev_max=$site_elev_max&site_temp_min=$site_temp_min&site_temp_max=$site_temp_max&site_precip_min=$site_precip_min&site_precip_max=$site_precip_max&sort_type=site_precip')\" ><b>Precip</b></a>";?></td>
		...
		?>
        <tr bgcolor="#ffffff">
          <td><a href="site.php?site_id=<? echo"$site_id";?>" target="_blank">Details</a><? if ( ($site_lat != '' && $site_long != '') || ($site_lat == 'NULL' && $site_long == 'NULL') || ($site_lat == '0' && $site_long == '0') ) { ?> - <a href="#google_map" onClick="javascript:myclick('<? echo"$site_id";?>')">Map</a><? } ?></td>
          <td><? if($site_url!=''){?>
            <a href="<? echo"$site_url";?>" target="_blank">
            <? } ?>
            <? echo"$site_name";?>
            <? if($site_url!=''){?>
            </a>
            <? } ?></td>
          <td><? echo"$site_lat";?></td>
          <td><? echo"$site_long";?></td>
          <td>Low: <? echo"$site_elevlow";?><br />
            High: <? echo"$site_elevhigh";?><br />
            Mean: <? echo"$site_elevmean";?></td>
          <td><? echo"$site_temp";?></td>
          <td><? echo"$site_precip";?></td>
          <td><? echo"$ecosystem_id";?></td>
          <td><? echo"$state_id";?><br />
          <? echo"$country_id";?></td>
        </tr>
        <? } ?> 
